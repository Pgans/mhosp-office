...
class Booking extends \yii\db\ActiveRecord
{
    ...
    public function validateBookingPeriod($attribute, $params)
    {
        $start = strtotime($this->booking_start);
        $end = strtotime($this->booking_end);
        $maxDuration = 3 * 24 * 60 * 60; // 3 days in seconds

        if ($end - $start > $maxDuration) {
            $this->addError($attribute, 'Booking period cannot exceed 3 days.');
        }
    }
    ...
    public function rules()
{
    return [
        ...
        [['booking_format'], 'required', 'message' => 'กรุณาเลือกประเภทการจอง'], // เพิ่มข้อความแจ้งเตือน
        ...
        [['booking_start', 'booking_end'], 'validateBookingPeriod'],
        ...
        [['departments_id'], 'exist', 'skipOnError' => true, 'targetClass' => Departments::className(), 'targetAttribute' => ['departments_id' => 'dep_id']],
    ...
    {
        return $this->hasOne(Departments::className(), ['dep_id' => 'deparments_id']);
    }
