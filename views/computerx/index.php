...
<div class="col-md-12">
    <div class="panel panel-primary">
        <div class="panel-heading" style="background: linear-gradient(90deg, #8BC34A, #64B5F6, #9933FF); color: #ffffff;">
            <i class="glyphicon glyphicon-list-alt"></i> ข้อมูลการซ่อมที่ไม่ผ่านเกณฑ์
        </div>
        <div class="panel-body">
            <div id="comsodx2"></div>
            <div class="table-responsive">
                ...
                $sql = "
                    WITH FilteredJobs AS (
                        SELECT 
                            ...
                            j.send_at,
                            j.repair_at,
                            j.send_by AS sender_name,
                            j.repair_by AS repairer_name,
                            t.type AS type_name,
                            j.type_id,
                            TIMESTAMPDIFF(HOUR, j.send_at, j.repair_at) AS response_time_hours,
                            TIMESTAMPDIFF(MINUTE, j.send_at, j.repair_at) AS response_time_minutes,
                            CASE
                                WHEN MONTH(j.send_at) >= 10 THEN YEAR(j.send_at) + 1
                                ELSE YEAR(j.send_at)
                    ...
                    FailedJobs AS (
                        ...
                        FROM 
                            FilteredJobs
                        WHERE 
                            (type_id in (10,13,23,40,45) AND response_time_minutes > 15) OR
                            (type_id in (4,33,46) AND response_time_minutes > 20) OR
                            (type_id in (11,24) AND response_time_minutes > 30) OR
                            (type_id in (20,43) AND response_time_minutes > 180)
                    )
                    SELECT 
                        fiscal_year AS ปีงบประมาณ,
                        send_at AS วันที่ส่งซ่อม,
                        detail AS รายการส่งซ่อม,
                        sender_name AS ผู้ส่งซ่อม,
                        repairer_name AS ผู้ซ่อม,
                        type_name AS ประเภท,
                        response_time_hours AS ชั่วโมงการซ่อม
                    FROM 
                        FailedJobs
                    ORDER BY 
                        fiscal_year DESC, send_at DESC;
                ";
