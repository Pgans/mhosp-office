	...
	public function actionExportdm2()
{
    ...
    // ✅ แยก date สำหรับ DatePicker (ไม่มีเวลา)
    $date1Display = isset($data['date1']) && !empty($data['date1'])
        ? $data['date1']
        : date('Y-m-d');

    $date2Display = isset($data['date2']) && !empty($data['date2'])
        ? $data['date2']
        : date('Y-m-d');

    // ✅ ใช้ใน SQL query
    $date1 = $date1Display . ' 00:01';
    $date2 = $date2Display . ' 23:59';
    ...
    try {
        ...
        Yii::error('SQL Error: ' . $e->getMessage(), 'exportdm2');
        throw new \yii\web\ServerErrorHttpException('เกิดข้อผิดพลาดในการดึงข้อมูล: ' . $e->getMessage());
    }
    ...
    return $this->render('exportdm2', [
        'dataProvider' => $dataProvider,
        'sql'          => $sql,
        'date1'        => $date1Display,  // ✅ Y-m-d สำหรับ DatePicker
        'date2'        => $date2Display,
        'date1Full'    => $date1,         // ✅ full สำหรับแสดงใน panel
        'date2Full'    => $date2,
    ]);
}
   ######################################################################################################################
