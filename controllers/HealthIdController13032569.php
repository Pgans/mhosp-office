...
class HealthIdController extends Controller
{
    ...
    protected function getProviderIdToken(string $healthIdToken): ?array
    {
        ...
        try {
            ...
            if ($httpCode === 200 && isset($data['data'])) {
                ...
                // decode Health ID JWT เพื่อดึง id_card จาก scopes_detail
                $jwtPayload   = $this->decodeJwtPayload($healthIdToken);
                $scopesDetail = $jwtPayload['scopes_detail'] ?? [];
                $idCard       = $scopesDetail['id_card'] ?? null;

                Yii::info('id_card from JWT: ' . ($idCard ?? 'null'), 'provider-id');
    ...
    protected function findOrCreateUser(array $providerData): ?User
    {
        ...
        $idCard           = $providerData['id_card']    ?? null;
        ...
        if ($providerRecord) {
            ...
            // อัปเดต last_login และ id_card
            $db->createCommand(
                'UPDATE provider_id_users SET last_login = NOW(), id_card = :ic WHERE username = :u',
                [
                    ':ic' => $idCard,
                    ':u'  => $providerUsername,
                ]
            ...
            Yii::info('Login สำเร็จ (existing): ' . $providerUsername . ' id_card=' . ($idCard ?? 'null'), 'provider-id');
            return $user;
        }
        ...
        // บันทึกลง provider_id_users พร้อม id_card
        ...
        Yii::info('Auto-created สำเร็จ: ' . $providerUsername . ' (user_id=' . $user->id . ') id_card=' . ($idCard ?? 'null'), 'provider-id');
        return $user;
    }
}