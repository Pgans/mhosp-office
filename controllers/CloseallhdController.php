...
class CloseallhdController extends \yii\web\Controller
{
            ...
            'rules' => [
                [
                    ...
                    'matchCallback' => function ($rule, $action) {
						...
						$allowedUsers = ['6']; // ตัวอย่าง user_id ที่ได้รับอนุญาต  6=pgans, 29=boom2518  toa=52  289=junmane 190=name  285=earth 286:gob 291=john 
...
FROM (
    ...
    WHERE a.REG_DATETIME BETWEEN CURDATE() AND NOW()
      AND a.is_cancel = 0
      AND p.NATN_ID = '99'
      AND a.unit_reg  IN ('42')  ###51=NCD คัดกรอง  42=HD
...
(SELECT @n := 0) AS init
ORDER BY 
    (data.unit_id = 24) DESC, data.amount DESC,    -- 🔹 ย้ำอีกชั้นเพื่อให้แน่ใจว่าอยู่บนสุดในลำดับสุดท้าย
    No ASC;
;
 ";
