...
##########################################################################
		...
		REPLACE(p.cid,substr(p.cid,6,3),'xxx') as hn,
        #p.hn as hn,
...
##########################################################################
        // ข้อมูลวันที่มารักษา
        $sqld = "SELECT 
		#date(s.date_serv) as tdate,
		date_format(date_add(s.date_serv, INTERVAL 543 YEAR),'%d-%m-%Y') tdate,
		time(s.time_serv) as tidate,
                ...
                WHERE  p.cid = '$cid'
				AND s.date_serv >= '20200101'
				AND s.SEQ NOT IN (SELECT dx.seq FROM diagnosis_opd dx , cicd10tm i WHERE dx.diagcode = i.diagcode AND i.diagcode = 'Z480')
                AND s.chiefcomp <> ''
		GROUP BY s.SEQ
                ORDER BY s.date_serv DESC LIMIT 15";
			    // WHERE  p.cid = '$cid'
...
##########################################################################
        ...
        $sqlcc = "SELECT 
					#date_serv,
					date_format(date_add(s.date_serv, INTERVAL 543 YEAR),'%d-%m-%Y') date_serv,
					CHIEFCOMP,sbp,dbp,pr,rr,btemp,h.hosname as hospname,
...
##########################################################################
        ...
        $sqldr = "SELECT  app_id,
				...
				d_update,
				cid
				FROM m30_appoints m3
				WHERE   seq ='$seq' LIMIT 1";
        $rawapp = $connection->createCommand($sqldr)
                ->queryAll();

        $dataProviderapp = new ArrayDataProvider([
            //'key' => 'hoscode',
            'allModels' => $rawapp,
            'pagination' => [
                'pageSize' => 20
...
##########################################################################
        ...
        $rawweb = $connection1->createCommand($sqlweb)
                ->queryAll();
