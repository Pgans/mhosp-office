...
<div class="filter-card">
    ...
    <div class="shortcut-group">
        ...
        <button type="button" class="btn-shortcut" onclick="setDateRange(1)">วันนี้</button>
...
<?php
// ✅ นับจำนวนที่อายุน้อยกว่า 65 ปี
$allModels = $dataProvider->allModels;
$countUnder65 = count(array_filter($allModels, function($row) {
    return isset($row['AGE_Y']) && (int)$row['AGE_Y'] < 65;
}));
$countTotal = count($allModels);
?>
<!-- ====== GRID ====== -->
...
echo GridView::widget([
    ...
    'before' => '<span style="font-size:14px;font-weight:600;color:#0a7a74">📋 รายการนำเข้า Ntip — โรคเบาหวาน</span>'
              . '&nbsp;&nbsp;'
              . '<span style="background:linear-gradient(135deg,#07a39e,#2ec4b6);color:#fff;border-radius:20px;padding:3px 14px;font-size:13px;font-weight:600;margin-left:8px">'
              . '👤 อายุ &lt; 65 ปี: ' . $countUnder65 . ' ราย'
              . '</span>'
              . '&nbsp;'
              . '<span style="background:#f0fdf9;border:1.5px solid #9edbd2;color:#0b6b65;border-radius:20px;padding:3px 14px;font-size:13px;font-weight:600">'
              . '📊 ทั้งหมด: ' . $countTotal . ' ราย'
              . '</span>',
    'after'  => '<span style="color:#e05c3a;font-weight:600">📆 ช่วงเวลา:</span> '
              . '<span style="color:#2b4b47">' . $date1Full . '</span>'
              . '<span style="color:#888;margin:0 6px">ถึง</span>'
              . '<span style="color:#2b4b47">' . $date2Full . '</span>',
],
    'columns' => [
        ...
        ['attribute' => 'AGE_Y',               'header' => 'AGE_Y',               'headerOptions' => ['style' => 'background:linear-gradient(180deg,#e0f5f1,#caeee8)']],
        ['attribute' => 'AGE_M',               'header' => 'AGE_M',               'headerOptions' => ['style' => 'background:linear-gradient(180deg,#e0f5f1,#caeee8)']],
...
function setDateRange(days) {
    ...
    // ✅ อัปเดต hidden input (ค่าจริงที่ส่ง POST)
    var h1 = document.querySelector('input[name="date1"]');
    var h2 = document.querySelector('input[name="date2"]');
    if (h1) h1.value = d1;
    if (h2) h2.value = d2;

    // ✅ อัปเดต visible input ของ jQuery UI DatePicker
    try { $('#date1').datepicker('setDate', d1); } catch(e) {}
    try { $('#date2').datepicker('setDate', d2); } catch(e) {}
    ...
    // ✅ auto-submit หลังเลือก shortcut
    setTimeout(function() {
