  ...
  public function actionIndex()
{
    ...
    $visitProvider = new \yii\data\ArrayDataProvider([
		...
		########## ขอ Authencode ######################################################
		$todays = "SELECT
			COUNT(DISTINCT CASE WHEN DATE(b.reg_datetime) = CURDATE() THEN b.visit_id END) AS todayopd,
			COUNT(DISTINCT CASE WHEN p.NATN_ID != 99 THEN b.visit_id END) AS alien,
			COUNT(DISTINCT CASE WHEN e.unit_id IN (999, 888) THEN b.visit_id END) AS homeward,
			COUNT(DISTINCT b.visit_id) AS totalopd,
			COUNT(DISTINCT ak.claimcode) AS authen,
			COUNT(DISTINCT CASE WHEN ak.claimcode IS NULL THEN b.visit_id END) AS noauthen,
			COUNT(DISTINCT cl.claimcode) AS closevisit,
			COUNT(DISTINCT CASE WHEN cl.claimcode IS NULL THEN b.visit_id END) AS noclosevisit,
		    COUNT(DISTINCT CASE WHEN lc.status = '200' THEN b.visit_id END) AS jongclaim,
		    COUNT(DISTINCT CASE WHEN lc.status IS NULL THEN b.visit_id END) AS nojongclaim
		FROM opd_visits b
        ...
        return $this->render('index', [
			...
			'authenj' => $authenj,
			'noauthenj' => $noauthenj,
			//'visitProvider' => $this->getVisitData(),
