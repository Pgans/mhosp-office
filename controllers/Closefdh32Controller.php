...
";
    ...
    $visitProvider = new \yii\data\ArrayDataProvider([
       ...
       'sort' => [
        ...
        $data = \yii::$app->db74->createCommand($sqlCount0)->queryAll();
        ...
        $data = \yii::$app->db4->createCommand($sqlCount1)->queryAll();
        for ($i = 0; $i < sizeof($data); $i++) {
...
INNER JOIN population p ON c.CID = p.CID
LEFT JOIN prescriptions ps ON ps.visit_id = b.visit_id AND ps.is_cancel = 0
LEFT JOIN drugs dr ON dr.drug_id = ps.drug_id 
...
*/		
        ...
        AND p.natn_id <> '99'";
        $data = \yii::$app->db4->createCommand($cidalien)->queryOne();
        $alien = $data['alienx']; 
...
public function actionCheck1()
{
    ...
    $data = \Yii::$app->db74->createCommand($sqltoken)->queryOne();
