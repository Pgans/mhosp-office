...
class AuthenyController extends \yii\web\Controller
{
	/*
...
FROM (
    ...
    WHERE b.IS_CANCEL = 0
      AND b.REG_DATETIME AND b.REG_DATETIME BETWEEN '$date1' AND '$date2'
      AND b.UNIT_REG NOT IN ('42','51')
      AND p.NATN_ID = '99'
	  AND m.inscl NOT IN ('17','29','32','34')
      AND (ak.claimcode IS NULL OR ak.claimcode = '')
      /* ✅ ตัดเคสที่ cid ขึ้นต้นด้วยศูนย์ >= 5 ตัว หรือ telephone ไม่ใช่ตัวเลข */
      AND NOT (
          p.cid REGEXP '^0{5,}' 
          OR p.telephone IS NULL 
          OR TRIM(p.telephone) = '' 
          OR TRIM(p.telephone) = 'ไม่มี'
          OR TRIM(p.telephone) NOT REGEXP '^[0-9]+$'
      )
    GROUP BY b.VISIT_ID 
