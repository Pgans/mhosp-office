...
$this->title = 'ข้อมูลการปิดสิทธิ์ทุกแผนก';
...
<div class="card">
 ...
 <div style="font-size: 2.2rem; font-weight: bold; color: deeppink;">
        ตรวจสอบการขอ AuthCode-ปิดสิทธิ์ทุกแผนก สำหรับ Admin
    </div>
        ...
        <tbody>
            <?php if ($visitProvider && $visitProvider->getCount() > 0): ?>
                <?php foreach ($visitProvider->getModels() as $key => $value) : ?>
                    <tr id="row-<?= $value["visit"]; ?>" onclick="rememberRow('row-<?= $value["visit"]; ?>')">
                        <td><?= $value["No"]; ?></td>
                        <td><?= $value["regdate"]; ?></td>
                        <td><?= $value["visit"]; ?></td>
                        <td><?= $value["cid"]; ?></td>
...
<?php
$this->registerJs("
    function rememberRow(rowId) {
        localStorage.setItem('highlightedRowId', rowId);
    }

    window.onload = function () {
        const rowId = localStorage.getItem('highlightedRowId');
        if (rowId) {
            const targetRow = document.getElementById(rowId);
            if (targetRow) {
                targetRow.scrollIntoView({ behavior: 'smooth', block: 'center' });
                targetRow.style.backgroundColor = '#fff6cc'; // ไฮไลต์พื้นหลังแถว
                setTimeout(() => {
                    targetRow.style.transition = 'background-color 1s';
                    targetRow.style.backgroundColor = '';
                }, 3000); // ลบไฮไลต์หลัง 3 วินาที
            }
        }
    };
");
