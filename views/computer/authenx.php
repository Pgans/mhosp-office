...
<style>
    ...
    th {
        background: linear-gradient(to bottom right, #e6ccff, #d1b3ff); /* สีม่วงอ่อนแบบ gradient */
        color: #4a0072; /* สีม่วงเข้มสำหรับข้อความ */
        padding: 10px;
        text-align: center;
        font-weight: bold;
    ...
    th:hover {
        background: linear-gradient(to bottom right, #f0e6ff, #e6ccff); /* เพิ่มแสงสดใสเมื่อ hover */
    }
</style>
</style>
<div class="panel-body">
    <div class="panel panel-warning">
        <div class="panel-heading"><i class="glyphicon glyphicon-plus-sign"></i> สรุปการขอ AuthenCode</<i></div>
        <div class="panel-body">
            <div class="row">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        COUNT(DISTINCT b.VISIT_ID) AS visit,  -- ตัดซ้ำด้วย DISTINCT
                        COUNT(DISTINCT ak.claimcode) AS authencode,  -- ตัดซ้ำในกรณีนี้ด้วย DISTINCT
                        ...
                        ROUND((COUNT(DISTINCT b.VISIT_ID) - COUNT(DISTINCT ak.claimcode)) / COUNT(DISTINCT b.VISIT_ID) * 100, 2) AS percent_non_authen
                    FROM opd_visits b 
                    ...
                    <tbody>
                        ...
                        FROM (
                            SELECT 
                                ...
                                'OPD' AS type,
                                COUNT(*) AS count
                            FROM mbase_data1.log_fdh_opd_ck
                            ...
                            SELECT 
                                ...
                                'IPD' AS type,
                                COUNT(*) AS count
                            FROM mbase_data1.log_fdh_ipd_ck
                            WHERE d_update BETWEEN CURDATE() - INTERVAL 15 DAY AND NOW() AND  messages <> ''
                            GROUP BY DATE(d_update)
                        ) AS log
                        GROUP BY DATE(d_update)
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT 
                            ...
                            COUNT(*) AS 'total'
                        FROM 
                            mbase_data1.log_fdh_opd_ck AS log
                        WHERE 
                            log.d_update BETWEEN CURDATE() - INTERVAL 25 DAY AND NOW() AND  log.messages <> ''
                        GROUP BY 
                            DATE(log.d_update)
                        ORDER BY 
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        COUNT(*) AS 'total'
                        FROM 
                            mbase_data1.log_fdh_ipd_ck AS log
                        WHERE 
