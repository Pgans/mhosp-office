...
$sql = "SELECT 
		...
		FROM (
			...
			LEFT JOIN log_closevisits lc ON lc.visit_id = a.visit_id  
...
$visitProvider = new \yii\data\ArrayDataProvider([
   ...
   'sort' => [
	...
	$data = \yii::$app->db2->createCommand($sqlCount0)->queryAll();
	...
	for ($i = 0; $i < sizeof($data); $i++) {
	 ...
	 $sqlCamount = "SELECT 
    IFNULL(COUNT(DISTINCT o.visit_id), 0) AS amountx
FROM opd_visits o
...
COUNT(DISTINCT ak.claimcode) AS authen,
COUNT(DISTINCT b.VISIT_ID) - COUNT(DISTINCT ak.claimcode) AS noauthen,
COUNT(DISTINCT cl.visit_id) AS closevisits,
COUNT(DISTINCT b.VISIT_ID) - COUNT(DISTINCT cl.claimcode) AS noclosevisit,
...
LEFT JOIN authen_kiosk ak ON b.visit_id = ak.visit_id AND p.cid = ak.cid
...
AND b.UNIT_REG NOT IN ('42','51','79')
...
COUNT(DISTINCT CASE WHEN k.claimcode_nhso IS NOT NULL AND k.claimcode_nhso <> '' THEN k.seq END) AS authenj,
COUNT(DISTINCT CASE WHEN k.claimcode IS NOT NULL AND k.claimcode <> '' THEN k.seq END) AS closej,
...
WHERE DATE(v.visitdate) BETWEEN CURDATE() AND NOW()
) AS k
...
ORDER BY regdate DESC
	...
	for ($i = 0; $i < sizeof($data); $i++) {
		...
		//$noclosevisit = $data[$i]['noclosevisit'];
		$jongclaimj = $data[$i]['claimj'];
		$nojongclaimj = $data[$i]['noclaimj'];
...
// ✅ แปลง JSON เป็น array ก่อน
$closevisit = json_decode($response, true);

// ตรวจสอบว่า JSON ถูกต้อง
if (!is_array($closevisit)) {
Yii::$app->session->setFlash('error', '❌ API ส่งข้อมูลผิดพลาด: ' . $response);
return $this->redirect(['index']);
}

// ✅ ตรวจสอบว่า API ส่ง dataError หรือไม่
if (isset($closevisit['dataError']) && !empty($closevisit['dataError'])) {
Yii::$app->session->setFlash('error', '❌ ' . $closevisit['dataError']);
return $this->redirect(['index']);
}
...
$transaction_id = $closevisit['data']['transaction_id'] ?? "";
$claimcode = $closevisit['data']['authen_code'] ?? "";

// ✅ เช็คว่าค่าจำเป็นครบก่อนบันทึก
...
// Yii::$app->session->addFlash('success', "✅ บันทึกข้อมูลสำเร็จ! cid: {$closeRow['cid']}, claimCode: {$claimcode}");
} else {
Yii::$app->session->setFlash('error', '❌ ไม่สามารถบันทึกข้อมูลได้: ข้อมูลไม่ครบจาก API มีในระบบแล้ว ');
}

#return $this->redirect(['index']);
return $this->redirect(['index', 'highlight' => $visit]);
}
/*
// ตรวจสอบว่าได้ข้อมูลที่จำเป็นจาก API เพื่อบันทึก
$message = isset($closevisit['message']) ? addslashes($closevisit['message']) : "";
$message_th = isset($closevisit['message_th']) ? addslashes($closevisit['message_th']) : "";
...
// เงื่อนไขการบันทึก: ต้องมี message และ transaction_id
if (!empty($message) && !empty($transaction_id)) {
$strSQL = "INSERT INTO close_visits (
	visit_id, cid, transaction_id, claimtype, claimcode, recorder_pid, message, claim_datetime, total_amount, d_update
) VALUES (
...
// เพิ่ม Flash message เมื่อบันทึกสำเร็จ
...
// ไม่บันทึกข้อมูล แต่แจ้งเตือนว่าข้อมูลไม่ครบ
Yii::$app->session->setFlash('error', '❌ ไม่สามารถบันทึกข้อมูลได้: ข้อมูลไม่สมบูรณ์');
}
...
public function actionCheckNhso($cid, $visit_id)
{
    ...
    // ถ้าเป็น object เดียว แปลงให้เป็น array
    ...
    if (!empty($epClaims)) {
        // ดึงเวลา d_update ของ visit_id จาก close_visits
        $dUpdate = Yii::$app->db2->createCommand("
            SELECT MAX(d_update) 
            FROM close_visits 
        ...
        // ถ้าไม่มี d_update ให้ใช้เวลาปัจจุบัน
        $dUpdateTime = $dUpdate ? date('H:i', strtotime($dUpdate)) : date('H:i');

        // หา EP claim ที่เวลาจาก claimDateTime ใกล้เคียงกับ d_update ที่สุด
        $minDiff = PHP_INT_MAX;
        foreach ($epClaims as $claim) {
            $claimTime = date('H:i', strtotime($claim['claimDateTime']));
            $diff = abs(strtotime($claimTime) - strtotime($dUpdateTime));
            if ($diff < $minDiff) {
                $minDiff = $diff;
                $selectedClaim = $claim;
            }
    ...
    if ($selectedClaim) {
        $claimType     = $selectedClaim['claimType'] ?? null;
        $telephone     = $selectedClaim['telephone'] ?? '';
        $claimCode     = $selectedClaim['claimCode'] ?? null;
        $claimDateTime = $selectedClaim['claimDateTime'] ?? null;

        // ตรวจสอบว่ามี visit_id อยู่แล้วหรือไม่
        $existingVisit = Yii::$app->db2->createCommand("
            SELECT COUNT(*) 
            FROM close_visits 
            WHERE visit_id = :visit_id
        ")
        ...
        if ($existingVisit > 0) {
            // Update แถวที่มี visit_id อยู่แล้ว
            ...
            ->bindValues([
                ...
                ':d_update'       => $claimDateTime,
                ':visit_id'       => $visit_id,
            ])
            ->execute();

            Yii::$app->session->addFlash('info', "✅ Update ข้อมูลเรียบร้อย! visit_id: $visit_id, claimCode: $claimCode");
        } else {
            // Insert ข้อมูลใหม่
            Yii::$app->db2->createCommand("
                INSERT INTO close_visits (cid, visit_id, claimtype, claimcode, claim_datetime, d_update)
                VALUES (:cid, :visit_id, :claimtype, :claimcode, :claim_datetime, :d_update)
            ")
            ->bindValues([
                ':cid'            => $cid,
                ':visit_id'       => $visit_id,
            ...
            Yii::$app->session->addFlash('info', "✅ Insert ข้อมูลเรียบร้อย! visit_id: $visit_id, claimCode: $claimCode");
        }
    } else {
        Yii::$app->session->setFlash('error', 'ไม่พบการปิดสิทธิ์. cid: ' . $cid );
    }

    // ✅ กลับไปหน้า index ทุกครั้ง
    return $this->redirect(['index']);
}
...
LEFT JOIN authen_kiosk ak ON b.visit_id = ak.visit_id AND ak.cid = p.cid  AND is_cancel = 0
